  ...
  try {
    switch (action) {
      case "list": {
        ...
        const result = await databases.listDocuments(DATABASE_ID, collectionId, q);
      ...
      case "create": {
        const result = await databases.createDocument(DATABASE_ID, collectionId, documentId || ID.unique(), data);
        return NextResponse.json(result);
      }
      case "update": {
